...
import { classifyError, isWalletRejection } from "../../lib/transaction-messages";
...
describe("transaction state", () => {
  ...
  it("signatureOf surfaces the signature from any phase that carries one", () => {
    ...
    expect(signatureOf({ phase: "error", kind: "unknown", message: "m", signature: "c" })).to.equal(
      "c",
    );
    expect(signatureOf({ phase: "error", kind: "unknown", message: "m" })).to.equal(null);
...
describe("error classification", () => {
  it("recognizes wallet rejection as cancellation", () => {
    expect(isWalletRejection(new Error("User rejected the request"))).to.equal(true);
    expect(isWalletRejection(new Error("insufficient funds"))).to.equal(false);
  });

  it("classifies blockhash expiry", () => {
    expect(classifyError(new Error("TransactionExpiredBlockheightExceededError")).kind).to.equal(
      "blockhash_expired",
    );
  });

  it("classifies confirmation timeout without declaring failure certain", () => {
    const c = classifyError(new Error("Transaction was not confirmed: timed out"));
  ...
  it("classifies simulation failure with a devnet hint", () => {
    const c = classifyError(new Error("Transaction simulation failed: unknown account"));
    expect(c.kind).to.equal("simulation_failure");
    expect(c.message).to.match(/devnet/i);
  });

  it("classifies RPC connectivity failures", () => {
    expect(classifyError(new Error("Failed to fetch")).kind).to.equal("rpc_failure");
  });
