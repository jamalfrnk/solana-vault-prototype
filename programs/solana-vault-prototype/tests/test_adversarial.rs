...
fn program_id() -> Pubkey { Pubkey::from(solana_vault_prototype::id().to_bytes()) }
fn keypair_pubkey(kp: &Keypair) -> Pubkey { Pubkey::from(kp.pubkey().to_bytes()) }
fn spl_token_id() -> Pubkey { Pubkey::from(anchor_spl::token::ID.to_bytes()) }
fn ata_program_id() -> Pubkey { Pubkey::from(anchor_spl::associated_token::ID.to_bytes()) }
fn system_program_id() -> Pubkey { Pubkey::from(anchor_lang::system_program::ID.to_bytes()) }

fn make_mint_account(mint_authority: &Pubkey, decimals: u8) -> Account {
    let mut data = vec![0u8; 82];
    data[0] = 1; data[4..36].copy_from_slice(mint_authority.as_ref());
    data[44] = decimals; data[45] = 1;
...
fn make_token_account(owner: &Pubkey, mint: &Pubkey, amount: u64) -> Account {
    ...
    data[108] = 1;
    Account { lamports: 2_039_280, data, owner: spl_token_id(), executable: false, rent_epoch: u64::MAX }
}
...
fn find_user_position(vault_state: &Pubkey, user: &Pubkey, pid: &Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[USER_POSITION_SEED, vault_state.as_ref(), user.as_ref()], pid)
}

fn ata(owner: &Pubkey, mint: &Pubkey) -> Pubkey {
    let tp = spl_token_id(); let ap = ata_program_id();
    Pubkey::find_program_address(&[owner.as_ref(), tp.as_ref(), mint.as_ref()], &ap).0
}

fn build_svm() -> LiteSVM {
    let mut svm = LiteSVM::new();
    svm.add_program(program_id(), include_bytes!("../../../target/deploy/solana_vault_prototype.so")).unwrap();
    svm
}
...
fn make_initialize_ix(payer: Pubkey, pause_authority: Pubkey, mint: Pubkey,
    vault_state: Pubkey, vault_authority: Pubkey, custody: Pubkey) -> Instruction {
    Instruction::new_with_bytes(program_id(),
        ...
        solana_vault_prototype::accounts::Initialize {
            ...
            token_program: spl_token_id(), associated_token_program: ata_program_id(),
            system_program: system_program_id(),
        }.to_account_metas(None))
}

fn make_deposit_ix(user: Pubkey, vault_state: Pubkey, vault_authority: Pubkey, custody: Pubkey,
    user_token_account: Pubkey, user_position: Pubkey, mint: Pubkey, amount: u64) -> Instruction {
    Instruction::new_with_bytes(program_id(),
        &solana_vault_prototype::instruction::Deposit { amount }.data(),
        solana_vault_prototype::accounts::Deposit {
            user, vault_state, vault_authority, custody, user_token_account, user_position, mint,
            token_program: spl_token_id(), system_program: system_program_id(),
        }.to_account_metas(None))
}

fn make_withdraw_ix(user: Pubkey, vault_state: Pubkey, vault_authority: Pubkey, custody: Pubkey,
    user_token_account: Pubkey, user_position: Pubkey, mint: Pubkey, shares_in: u64) -> Instruction {
    Instruction::new_with_bytes(program_id(),
...
impl Vault {
    fn new() -> Self {
        ...
        svm.set_account(mint_pk, make_mint_account(&keypair_pubkey(&ma), 6)).unwrap();
        ...
        let ix = make_initialize_ix(keypair_pubkey(&payer), keypair_pubkey(&pa), mint_pk,
            vault_state_pda, vault_authority_pda, custody_ata);
        send_ok(&mut svm, &[ix], &[&payer, &pa], &payer);

        Self { svm, pid, payer, _pause_authority: pa, mint_pk, vault_state_pda, vault_authority_pda, custody_ata }
    }
}
...
fn test_deposit_missing_user_signature() {
    ...
    v.svm.set_account(user_ata, make_token_account(&user_pk, &v.mint_pk, 1_000_000)).unwrap();
    let (pos, _) = find_user_position(&v.vault_state_pda, &user_pk, &v.pid);

    let ix = make_deposit_ix(user_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, user_ata, pos, v.mint_pk, 1_000_000);
...
fn test_withdraw_missing_user_signature() {
    ...
    v.svm.set_account(user_ata_pk, make_token_account(&user_pk, &v.mint_pk, 2_000_000)).unwrap();
    let (pos, _) = find_user_position(&v.vault_state_pda, &user_pk, &v.pid);

    // Deposit first (with user signature)
    let dep_ix = make_deposit_ix(user_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, user_ata_pk, pos, v.mint_pk, 2_000_000);
    send_ok(&mut v.svm, &[dep_ix], &[&v.payer, &user_kp], &v.payer);

    // Withdraw WITHOUT user signature
    let wd_ix = make_withdraw_ix(user_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, user_ata_pk, pos, v.mint_pk, 2_000_000);
    let blockhash = v.svm.latest_blockhash();
...
fn test_deposit_wrong_vault_state() {
    ...
    let pa2 = Keypair::new();
    v.svm.set_account(mint2_pk, make_mint_account(&keypair_pubkey(&ma2), 6)).unwrap();
    let (vs2, _) = find_vault_state(&mint2_pk, &v.pid);
    let (va2, _) = find_vault_authority(&vs2, &v.pid);
    let custody2 = ata(&va2, &mint2_pk);
    let ix2 = make_initialize_ix(keypair_pubkey(&v.payer), keypair_pubkey(&pa2), mint2_pk, vs2, va2, custody2);
    ...
    v.svm.set_account(user_ata_v1, make_token_account(&user_pk, &v.mint_pk, 1_000_000)).unwrap();
    ...
    assert!(v.svm.send_transaction(tx).is_err(), "wrong vault_state must be rejected");
}
...
fn test_deposit_wrong_token_account_owner() {
    ...
    v.svm.set_account(user_a_ata, make_token_account(&user_a_pk, &v.mint_pk, 1_000_000)).unwrap();
    ...
    let ix = make_deposit_ix(user_b_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, user_a_ata, pos_b, v.mint_pk, 500_000);
    let blockhash = v.svm.latest_blockhash();
    ...
    assert!(v.svm.send_transaction(tx).is_err(), "wrong token account owner must be rejected");
}
...
fn test_withdraw_cross_user_position_substitution() {
    ...
    v.svm.set_account(ata_a, make_token_account(&user_a_pk, &v.mint_pk, amount_a)).unwrap();
    v.svm.set_account(ata_b, make_token_account(&user_b_pk, &v.mint_pk, amount_b)).unwrap();

    let (pos_a, _) = find_user_position(&v.vault_state_pda, &user_a_pk, &v.pid);
    let (pos_b, _) = find_user_position(&v.vault_state_pda, &user_b_pk, &v.pid);

    // Both deposit
    send_ok(&mut v.svm, &[make_deposit_ix(user_a_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_a, pos_a, v.mint_pk, amount_a)], &[&v.payer, &user_a], &v.payer);
    send_ok(&mut v.svm, &[make_deposit_ix(user_b_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_b, pos_b, v.mint_pk, amount_b)], &[&v.payer, &user_b], &v.payer);

    // user_b tries to withdraw using user_a's position (which has more shares)
    let ix = make_withdraw_ix(
        user_b_pk,
        v.vault_state_pda, v.vault_authority_pda, v.custody_ata,
        ata_b,
    ...
    let msg = Message::new_with_blockhash(&[ix], Some(&user_b_pk), &blockhash);
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&user_b]).unwrap();
    assert!(v.svm.send_transaction(tx).is_err(), "cross-user position substitution must fail");
}
...
fn test_deposit_wrong_token_program() {
    ...
    let user_ata_pk = ata(&user_pk, &v.mint_pk);
    v.svm.set_account(user_ata_pk, make_token_account(&user_pk, &v.mint_pk, 1_000_000)).unwrap();
    let (pos, _) = find_user_position(&v.vault_state_pda, &user_pk, &v.pid);
    ...
    let ix = Instruction::new_with_bytes(program_id(),
        ...
        solana_vault_prototype::accounts::Deposit {
            user: user_pk, vault_state: v.vault_state_pda, vault_authority: v.vault_authority_pda,
            custody: v.custody_ata, user_token_account: user_ata_pk, user_position: pos,
            mint: v.mint_pk,
    ...
    let msg = Message::new_with_blockhash(&[ix], Some(&user_pk), &blockhash);
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&user]).unwrap();
    assert!(v.svm.send_transaction(tx).is_err(), "fake token program must be rejected");
}
...
fn test_adversarial_repeated_deposits_withdrawals_consistent() {
    ...
    let ata_a = ata(&user_a_pk, &v.mint_pk);
    let ata_b = ata(&user_b_pk, &v.mint_pk);
    v.svm.set_account(ata_a, make_token_account(&user_a_pk, &v.mint_pk, large_amount)).unwrap();
    v.svm.set_account(ata_b, make_token_account(&user_b_pk, &v.mint_pk, large_amount)).unwrap();
    ...
    send_ok(&mut v.svm, &[make_deposit_ix(user_a_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_a, pos_a, v.mint_pk, large_amount)], &[&v.payer, &user_a], &v.payer);

    // Deposit B (proportional)
    send_ok(&mut v.svm, &[make_deposit_ix(user_b_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_b, pos_b, v.mint_pk, large_amount)], &[&v.payer, &user_b], &v.payer);
    ...
    // A withdraws all
    send_ok(&mut v.svm, &[make_withdraw_ix(user_a_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_a, pos_a, v.mint_pk, large_amount)], &[&v.payer, &user_a], &v.payer);

    let vs_acct = v.svm.get_account(&v.vault_state_pda).unwrap();
    let vs = VaultState::try_deserialize(&mut vs_acct.data.as_slice()).unwrap();
    assert_eq!(vs.total_assets, large_amount, "after A withdrawal, half remains");
    assert_eq!(vs.total_shares, large_amount, "after A withdrawal, half shares remain");

    // B withdraws all
    send_ok(&mut v.svm, &[make_withdraw_ix(user_b_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_b, pos_b, v.mint_pk, large_amount)], &[&v.payer, &user_b], &v.payer);
...
fn test_deposit_large_amount_no_overflow() {
    ...
    let ata_a = ata(&user_a_pk, &v.mint_pk);
    let ata_b = ata(&user_b_pk, &v.mint_pk);
    v.svm.set_account(ata_a, make_token_account(&user_a_pk, &v.mint_pk, large)).unwrap();
    v.svm.set_account(ata_b, make_token_account(&user_b_pk, &v.mint_pk, large)).unwrap();
    ...
    send_ok(&mut v.svm, &[make_deposit_ix(user_a_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_a, pos_a, v.mint_pk, large)], &[&v.payer, &user_a], &v.payer);
    ...
    send_ok(&mut v.svm, &[make_deposit_ix(user_b_pk, v.vault_state_pda, v.vault_authority_pda,
        v.custody_ata, ata_b, pos_b, v.mint_pk, large)], &[&v.payer, &user_b], &v.payer);

    let vs_acct = v.svm.get_account(&v.vault_state_pda).unwrap();
    let vs = VaultState::try_deserialize(&mut vs_acct.data.as_slice()).unwrap();
    assert_eq!(vs.total_assets, large * 2, "total_assets should be 2x large");
    assert_eq!(vs.total_shares, large * 2, "no overflow — shares should equal assets at 1:1");
}
